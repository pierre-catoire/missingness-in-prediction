...
methodsDict = {"MARG": {"fit": ccMethodsM6M7.fitMarginalisation, "predict": ccMethodsM6M7.predictMarginalisation}}
...
for modelLabel in modelsDict.keys():
    ...
    for i in range(nSim):
        ...
        MY = np.random.rand(len(dTrain)) < .5
        dTrain.loc[MY, 'Y'] = pd.NA
        dTrain["SET"] = "TRAIN"
        dTrain["ITER"] = i
...
save_path = os.path.join(save_dir, "saveDataDiscreteM6M7_MISSY.csv")
